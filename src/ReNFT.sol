...
contract ReNFT is IReNft, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function distributePayments(
         ...
         */
        ISuperToken paymentToken = resolver.getPaymentToken(paymentTokenIx);
        ...
        ISuperToken(paymentToken).transfer(
            _lendingRenting.lending.lenderAddress,
            sendLenderAmt
        );
    ...
    function handleRent(CallData memory _cd) private {
        ...
        for (uint256 i = _cd.left; i < _cd.right; i++) {
...

            {
                ...
                int96 flowrate = rentPrice / SECONDS_IN_DAY;
                paymentToken.createFlowFrom(
                    msg.sender,
                    address(this),
                    flowrate
                );
